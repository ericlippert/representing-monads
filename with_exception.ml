...
module Value = struct
  ...
  let rec to_string v =
    ...
    | Wrapped (Normal v) -> "Ex " ^ (to_string v)
end
...
let my_bind f =
  let binder t =
    match f, t with
    | (Function f, Wrapped Exception) -> t
    | (Function f, Wrapped (Normal v)) -> f v
    | _ -> failwith "type error: 'bind' applied to non-function or non-monad" in
  Function binder

let my_extract v =
  match v with
  | Wrapped Exception -> failwith "unhandled exception!"
  | Wrapped (Normal v) -> v
  | _ -> failwith "type error: extract applied to non-monad"
...
let my_is_exception v =
  match v with
  | Wrapped Exception -> Boolean true
  | _ -> Boolean false
...
let () =
  ...
  let unit_ = ident "unit" in
  let bind_ = ident "bind" in
  let extract_ = ident "extract" in
  let is_exception = ident "is_exception" in
  let exception_ = ident "exception" in
  ...
  let env = add env unit_ (Function my_unit) in
  let env = add env bind_ (Function my_bind) in
  let env = add env extract_ (Function my_extract) in
  let env = add env is_exception (Function my_is_exception) in
  let env = add env exception_ (Wrapped Exception) in
    ...
    let c = f => n =>
      if (less_than n 0)
        ...
        normal (
          if (less_than n 2)
          1
          (multiply n (extract (f (subtract n 1))))) in
    fix c *)
        ...
        (if_ (apply2 less_than n zero)
          ...
          (apply
            ...
            (if_ (apply2 less_than n two)
              one
              (apply2 multiply
                n (apply extract_ (apply f (apply2 subtract n one)))))))))
      (apply fix_ c) in

  let env = add env factorial_ (evaluate env factorial) in
  let minus_one = apply2 subtract zero one in
  let app1 = apply factorial_ minus_one in
  let app2 = apply factorial_ six in
  (* let app3 = apply extract_ app1 in *)
  Printf.printf "%s\n" (Value.to_string (evaluate env app1));
  Printf.printf "%s\n" (Value.to_string (evaluate env app2));
  (* Printf.printf "%s\n" (Value.to_string (evaluate env app3)); *)
